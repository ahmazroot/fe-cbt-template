...
export function P({ children, className }: TypographyProps) {
    return (
        <p className={cn("text-base text-slate-600 leading-relaxed", className)}>
            {children}
        </p>
    );
}
...
export function Mono({ children, className }: TypographyProps) {
    return (
        <code className={cn("relative rounded bg-slate-100 px-[0.3rem] py-[0.2rem] font-mono text-xs font-semibold text-slate-400", className)}>
            {children}
        </code>
    );
}
