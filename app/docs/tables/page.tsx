    ...
    React.useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const path = selectedCategory && !searchQuery
                    ? `category/${selectedCategory}?`
                    : `search?q=${searchQuery}&`;

                const response = await fetch(`https://dummyjson.com/products/${path}limit=${limit}&skip=${skip}&select=title,price,category,thumbnail,rating,stock`);
                const data = await response.json();

                // API doesn't support filter combined,
                // local filter on the results for UI consistency.
                if (searchQuery && selectedCategory) {
                    const filteredProducts = data.products.filter((p: ProductData) =>
    ...
    const columns: ColumnDef<ProductData>[] = [
        {
            ...
            cell: (item) => (
                <div className="flex items-center gap-3">
                    <div className="size-10 rounded-lg bg-slate-100 flex-shrink-0 overflow-hidden border border-slate-200">
        ...
        },
        {
            ...
            cell: (item) => (
                <Badge variant="outline" className={cn(
                    "font-medium",
                    item.stock < 20 ? "border-amber-200 bg-amber-50 text-amber-700" : "border-slate-200 text-slate-600"
                )}>
                    {item.stock} unit
                </Badge>
            )
    ...
    return (
        <section className="min-h-screen flex flex-col items-center justify-center space-y-12 py-20 px-4 bg-slate-50">
            <div className="text-center space-y-2">
                ...
                <Typography.Muted>Clean and dense patterns for displaying tabular data and navigation.</Typography.Muted>
            </div>
        ...
        <div className="flex justify-end gap-1">
            ...
            <Dialog>
                <DialogTrigger asChild>
                    <Button variant="ghost" className="text-slate-400 hover:text-primary-600 p-2 rounded hover:bg-slate-100 h-9 w-9">
                ...
                <DialogContent className="sm:max-w-[425px]">
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="title" className="text-right text-sm font-medium text-slate-600">Judul</Label>
                            <Input id="title" defaultValue={item.title} className="col-span-3 h-9" />
                        </div>
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="price" className="text-right text-sm font-medium text-slate-600">Harga</Label>
                            <Input id="price" defaultValue={item.price} className="col-span-3 h-9" />
                        </div>
                    </div>
                    <DialogFooter>
                        <Button type="submit" className="bg-primary-600 hover:bg-primary-700">Simpan Perubahan</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog>
                <DialogTrigger asChild>
                    <Button variant="ghost" className="text-slate-400 hover:text-red-600 p-2 rounded hover:bg-slate-100 h-9 w-9">
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tindakan ini tidak dapat dibatalkan. Ini akan menghapus produk **{item.title}** secara permanen.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button type="submit" variant="destructive">Hapus Data</Button>
                        <DialogClose asChild>
    ...
    return (
        <Dialog>
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="new-title" className="text-right text-sm font-medium text-slate-600">Judul</Label>
                        <Input id="new-title" placeholder="Nama Produk" className="col-span-3 h-9" />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                        <Label htmlFor="new-price" className="text-right text-sm font-medium text-slate-600">Harga</Label>
                        <Input id="new-price" placeholder="0.00" className="col-span-3 h-9" />
                    </div>
                </div>
                <DialogFooter>
                    <Button type="submit" className="bg-emerald-600 hover:bg-emerald-700">Tambah Sekarang</Button>
                    <DialogClose asChild>
                            ...
                            {categories.map((cat) => (
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        <div className={cn(
                                            "size-4 border rounded-sm flex items-center justify-center",
                                            selectedCategory === cat ? "bg-primary-600 border-primary-600" : "border-slate-300"
                                        )}>
